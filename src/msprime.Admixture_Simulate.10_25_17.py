...
usage = "usage: python <script> -p nonAfr -o Tenn -s 1 -i 2 -n 0.02 -d 0.0 -t 350 -c F4Dstat -l 1e6 -e 1006 -a 1008"
...
parser.add_option("-c", "--calls_or_stats", action="store", type = "string", dest = "haplo", default="F4Dstat", help="Pick haplotype calls ('haplo') or 'F4Dstat' output, or 'debug'; default=F4Dstat")
...
if (options.outdir == "Tenn"):
	...
	simulation = msprime_demo_models.Tenn_demography(1, 1, 1, options.EU_sample_size, options.AS_sample_size, options.pulses, options.seed, options.n1_admix_prop, options.n2_admix_prop, options.outdir, options.t_n1_n2, options.haplo, options.length)

elif (options.outdir == "Sriram"):
	simulation = msprime_demo_models.Sriram_demography(1, 1, 1, options.EU_sample_size, options.AS_sample_size, options.pulses, options.seed, options.n1_admix_prop, options.n2_admix_prop, options.outdir, options.t_n1_n2, options.haplo, options.length)

elif (options.outdir == "SplitPop"):
	simulation = msprime_demo_models.SplitPop_demography(1, 1, 1, options.EU_sample_size, options.AS_sample_size, options.pulses, options.seed, options.n1_admix_prop, options.n2_admix_prop, options.outdir, options.t_n1_n2, options.haplo, options.length)
...
if (options.haplo == "haplo"):
	...
	nonAfr_samples = range(3, (EU_count + AS_count + 3))   # 2017 <- 1006 + 1008 + 3
	EU_samples = range(3, (EU_count + 3))   # 1009 <- 1006 + 3 
	AS_samples = range((EU_count + 3), (EU_count+AS_count+3))   # 1009 to 2017 <- (1006 + 3) to (1006 + 1008 + 3)
	Chimp_samples = range((EU_count+AS_count+3), (EU_count+AS_count+3)+1) # 2017 to 2018
	Deni_samples = range((EU_count+AS_count+3) + 1, (EU_count+AS_count+3) + 1 + 1)	#2018 to 2019
	if (options.pop == "EAS"):
...
##  FOR EACH SIMULATED CHROMOSOME, PRINT ALL THE INTROGRESSED HAPLOTYPES BELONGING TO THE SPECIFIED NON-AFR POPULATION IN BED FORMAT  ###
	for t, ts in enumerate(simulation):
		...
		# We are interested in coalescence events that occured in the Neandertal population
	        if record.population == 0 or record.population == 1:
...
#			print("\t", left, right, samples)
	...
	#BEDFILE_SORTED_MERGED = pybedtools.BedTool.sort(BEDFILE)
